...
const StatCard = memo(({ label, value, unit, icon: Icon, accent }: {
  ...
  <motion.div
    ...
    className="relative p-5 glass-card overflow-hidden"
  >
...
const ChartTooltip = ({ active, payload, label }: { active?: boolean; payload?: { value: number }[]; label?: string }) => {
  ...
  return (
    <div className="px-3 py-2 bg-[var(--card-bg-solid)] border border-[var(--border-color)] rounded-lg text-xs shadow-xl">
      <p className="text-[var(--text-muted)] mb-0.5">{label}</p>
      <p className="font-bold text-cyan-500">{fmt(payload[0].value)} km/h</p>
    </div>
  );
};
    ...
    <div className="w-full space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center space-x-3">
          <Satellite className="text-cyan-500 w-7 h-7" />
          <h2 className="text-2xl font-bold text-[var(--text-primary)] tracking-wider">ISS Telemetry</h2>
          <div className="flex items-center px-3 py-1 bg-green-500/10 border border-green-500/20 rounded-full">
            ...
            <span className="text-[11px] font-semibold text-green-500 uppercase tracking-wider">Live · 15s</span>
          </div>
        </div>
        <motion.button
          whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
          onClick={refresh} disabled={loading}
          className="flex items-center space-x-2 px-4 py-2 bg-[var(--card-bg)] hover:bg-slate-500/5 border border-[var(--border-color)] rounded-lg text-sm font-medium text-[var(--text-secondary)] transition-colors disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Latitude" value={currentData ? currentData.latitude.toFixed(4) : '—'} unit="°" icon={MapPin} accent="text-[var(--text-primary)]" />
        <StatCard label="Longitude" value={currentData ? currentData.longitude.toFixed(4) : '—'} unit="°" icon={MapPin} accent="text-[var(--text-primary)]" />
        <StatCard label="Current Speed" value={currentData?.speed ? fmt(currentData.speed) : '—'} unit="km/h" icon={Zap} accent="text-cyan-500" />
        <StatCard label="Avg Speed" value={avgSpeed > 0 ? fmt(avgSpeed) : '—'} unit="km/h" icon={Activity} accent="text-purple-500" />
      </div>
      ...
      >
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center space-x-4 text-xs text-[var(--text-muted)]">
            <span>Max: <span className="text-purple-500 font-mono font-semibold">{maxSpeed > 0 ? fmt(maxSpeed) : '—'} km/h</span></span>
            <span>Points: <span className="text-cyan-500 font-mono font-semibold">{trajectory.length}/30</span></span>
          </div>
        </div>
          ...
          {chartData.length > 1 ? (
            <ResponsiveContainer width="100%" height="100%" minHeight={256}>
              <LineChart data={chartData}>
                <XAxis dataKey="time" stroke="var(--text-muted)" fontSize={11} tickMargin={8} interval="preserveStartEnd" axisLine={false} tickLine={false} />
                ...
                <Line
                  ...
                  stroke="var(--accent-cyan)" strokeWidth={3}
                  dot={false}
